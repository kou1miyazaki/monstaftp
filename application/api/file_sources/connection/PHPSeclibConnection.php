    ...
    class PHPSeclibConnection extends SFTPConnectionBase {
        ...
        protected function handleDownloadFile($transferOperation) {
            ...
            if($transferSuccess)
                ...
                mftpLog(LOG_WARNING, "SFTPSecLib failed to get '{$transferOperation->getRemotePath()}' to '{$transferOperation->getLocalPath()}': {$this->connection->getLastSFTPError()}");
        ...
        protected function handleUploadFile($transferOperation) {
            ...
            if($transferSuccess)
                ...
                mftpLog(LOG_WARNING, "SFTPSecLib failed to put '{$transferOperation->getRemotePath()}' to '{$transferOperation->getLocalPath()}': {$this->connection->getLastSFTPError()}");

            if(!is_null($transferOperation->getCreateMode()))
        ...
        protected function handleDeleteFile($remotePath) {
            ...
            if($deleteSuccess)
                ...
                mftpLog(LOG_WARNING, "SFTPSecLib failed to delete file '$remotePath': {$this->connection->getLastSFTPError()}");
        ...
        protected function handleMakeDirectory($remotePath) {
            ...
            if($createSuccess)
                ...
                mftpLog(LOG_WARNING, "SFTPSecLib failed to create directory '$remotePath': {$this->connection->getLastSFTPError()}");
        ...
        protected function handleDeleteDirectory($remotePath) {
            ...
            if($deleteSuccess)
                ...
                mftpLog(LOG_WARNING, "SFTPSecLib failed to directory directory '$remotePath': {$this->connection->getLastSFTPError()}");
        ...
        protected function handleRename($source, $destination) {
            ...
            if($renameSuccess)
                ...
                mftpLog(LOG_WARNING, "SFTPSecLib failed to rename ''$source' to '$destination': {$this->connection->getLastSFTPError()}");
        ...
        protected function handleChangePermissions($mode, $remotePath) {
            ...
            if($operationSuccess)
                ...
                mftpLog(LOG_DEBUG, sprintf("SFTPSecLib failed to change perms of '%s' to '%o': ", $remotePath, $mode, $this->connection->getLastSFTPError()));
